 ...
 * registry.
 *
 * Takes a tenant rather than resolving one so it can be called from the
 * `endpoint()` route, from a script, or from a future automation — auth and
 * permission are the caller's job.
 */
export async function syncTablesForTenant(
  tenant: TenantContext
): Promise<SyncResult> {
  {
