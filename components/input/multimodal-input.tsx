...
import { extractUrls, useUrlDetection } from "@/hooks/use-url-detection";
...
import { PromptInput, PromptInputSubmit } from "../elements/prompt-input";
...
import { buildContextItems, ContextTray } from "./context-tray";
...
function PureMultimodalInput({
  ...
  const submitForm = useCallback(() => {
    ...
    const validMentions = mentions.filter((mention) => {
      // Basic validation
      if (!mention.type || !mention.label) {
        return false;
      }
      // Type-specific validation
      if (mention.type === "table" && !("tableName" in mention)) {
        return false;
      }
      if (
        mention.type === "record" &&
        (!("tableName" in mention) || !("recordId" in mention))
      ) {
        return false;
      }
      if (mention.type === "block" && !("blockId" in mention)) {
        return false;
      }
      return true;
    });
  ...
  const handleRemoveContextItem = useCallback(
    (id: string) => {
      if (id.startsWith("skill-")) {
        ...
        setAttachments((prev) =>
          prev.filter((a) => a.url !== url && a.name !== url)
        );
        ...
        >
              ...
              {(uploadQueue.length > 0 || isLoadingUrls) && (
                <div className="mb-2 flex items-center gap-2">
                  {uploadQueue.map((filename) => (
                    <div
                      className="flex h-8 items-center gap-1.5 rounded-lg border border-border bg-muted/50 px-2.5"
                      key={filename}
                    >
                      <Loader size={14} />
                  ...
                  {!!isLoadingUrls && (
                    <div className="flex h-8 items-center gap-1.5 rounded-lg border border-indigo-500/30 bg-indigo-500/5 px-2.5">
                      <Loader size={14} />
          ...
          <div className="flex items-center justify-between gap-2 px-3 py-2">
            <div className="flex items-center gap-1">
              ...
              />
              <Button
                className="flex aspect-square h-7 w-7 items-center justify-center rounded-md p-0 text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
                ...
                variant="ghost"
              >
                <AtSign size={12} style={{ height: 12, width: 12 }} />
              </Button>
              <Button
                className="flex aspect-square h-7 w-7 items-center justify-center rounded-md p-0 text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
                ...
                variant="ghost"
              >
                <Zap size={12} style={{ height: 12, width: 12 }} />
              </Button>
            </div>
...
export const MultimodalInput = memo(
  ...
  (prevProps, nextProps) => {
    ...
    if (prevProps.onOptimisticMessage !== nextProps.onOptimisticMessage) {
      return false;
    }
...
function PureAttachmentsButton({
  ...
  return (
    <Button
      ...
      variant="ghost"
    >
      <PaperclipIcon size={12} style={{ height: 12, width: 12 }} />
    </Button>
  );
}
