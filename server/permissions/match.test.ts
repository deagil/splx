...
describe("checkPermissionAgainstMap", () => {
  it("denies unknown roles rather than defaulting open", () => {
    expect(checkPermissionAgainstMap(staticMap, ["nonexistent"], "pages.view")).toBe(
      false
    );
  ...
  it("grants builder the reports and chat permissions the old static map omitted", () => {
    ...
    expect(checkPermissionAgainstMap(staticMap, ["builder"], "reports.view")).toBe(
      true
    );
    expect(checkPermissionAgainstMap(staticMap, ["builder"], "reports.edit")).toBe(
      true
    );
    expect(checkPermissionAgainstMap(staticMap, ["builder"], "chat.create")).toBe(
      true
    );
  });
  ...
  it("keeps viewer read-only", () => {
    ...
    expect(checkPermissionAgainstMap(staticMap, ["viewer"], "data.create")).toBe(
      false
    );
    expect(checkPermissionAgainstMap(staticMap, ["viewer"], "data.edit")).toBe(
      false
    );
    expect(checkPermissionAgainstMap(staticMap, ["viewer"], "data.delete")).toBe(
      false
    );
  ...
  it("does not grant non-admins workspace administration", () => {
    for (const role of ["builder", "user", "viewer"]) {
      expect(checkPermissionAgainstMap(staticMap, [role], "workspace.users")).toBe(
        false
      );
      expect(
  ...
  it("canonicalises the permission strings routes gated on but nothing granted", () => {
    ...
    expect(checkPermissionAgainstMap(staticMap, ["viewer"], "data.update")).toBe(
      false
    );
  });
